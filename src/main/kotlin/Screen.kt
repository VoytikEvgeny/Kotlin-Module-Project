class Screen {
    fun start(){
        val setOfArch: MutableSet<SomeItem> = mutableSetOf()
        createScreen(setOfArch, Archive(), "")

    }
    private fun createScreen(set:MutableSet<SomeItem>, folder: SomeItem, parentName: String){
        do {
            ...
            when (command) {
                ...
                in 1..menuList.size - 2 -> {
                    val element: SomeItem = set.elementAt(command - 1)
                    element.command()
                }
    ...
    private fun itemAdd(folder: SomeItem, input: String): SomeItem {
    ...
    private fun SomeItem.instanceGet(name:String):SomeItem{
        return when (this){
            is Archive -> Archive(name)
            else -> Note(name)
        }
    }
    private fun SomeItem.command() {
        when (this){
